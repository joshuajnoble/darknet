...
//image get_image_from_stream_kinect(IInfraredFrameReader *cap, UINT16* irBuffer, int w, int h, int c, IplImage** in_img, int cpp_video_capture, int dont_close);
...
void initializeKinect() {
    ...
    kinect->lpVtbl->Open(kinect);
    BOOLEAN isOpen;
    kinect->lpVtbl->get_IsOpen(kinect, &isOpen);
    if (!isOpen) {
        printf("omg failed IKinectSensor::get_IsOpen( &isOpen )");
    }

    //kinect->lpVtbl->get_CoordinateMapper(&coordinateMapper);
}

IInfraredFrameReader* infraredFrameReader = NULL;
BOOLEAN infrared_initialized = 0;
int infraredWidth;
int infraredHeight;
...
void initializeInfraredFrame() {
    ...
    kinect->lpVtbl->get_InfraredFrameSource(kinect, &infraredFrameSource);
    infraredFrameSource->lpVtbl->OpenReader(infraredFrameSource, &infraredFrameReader);
    IFrameDescription* infraredFrameDescription;
    infraredFrameSource->lpVtbl->get_FrameDescription(infraredFrameSource, &infraredFrameDescription);
    infraredFrameDescription->lpVtbl->get_Width(infraredFrameDescription, &infraredWidth);
    infraredFrameDescription->lpVtbl->get_Height(infraredFrameDescription, &infraredHeight);
    //cout << "infrared: " << infraredWidth << " " << infraredHeight << endl;
    //infraredBuffer->lpVtbl->resize(infraredWidth * infraredHeight);
    infraredBuffer = malloc(infraredWidth * infraredHeight, sizeof(UINT16));
    infrared_initialized = 1;
}
void updateInfraredFrame() {
    if (infrared_initialized == 0) {
        initializeInfraredFrame();
    }
    IInfraredFrame* infraredFrame;
    HRESULT ret = infraredFrameReader->lpVtbl->AcquireLatestFrame(infraredFrameReader, &infraredFrame);
    if (FAILED(ret)) {
        return;
    }
    infraredFrame->lpVtbl->CopyFrameDataToArray(infraredFrame, sizeof(infraredBuffer), &infraredBuffer[0]);
}
...
void *fetch_in_thread_kinect(void *ptr)
{
    //in = get_image_from_stream(cap);
    int dont_close_stream = 0;    // set 1 if your IP-camera periodically turns off and turns on video-stream
    
    //in_s = get_image_from_stream_kinect(infraredFrameReader, &infraredBuffer[0], net.w, net.h, net.c, &in_img, cpp_video_capture, dont_close_stream);

    image in;
    in.data = malloc(424 * 512 * sizeof(UINT16));
    in.h = 424;
    in.w = 512;
    in.c = 1;

    // pass a pointer so that we can either get a new kinect frame or optionally fail and return an error code
    int kinectStreamResult = get_image_from_stream_kinect(&in, infraredFrameReader, &infraredBuffer[0], net.w, net.h, net.c, &in_img, cpp_video_capture, dont_close_stream);

    if (kinectStreamResult > 0) // did it work?
    {
        in_s = resize_image(in, net.w, net.h);
        return 1;
    }
    printf("no kinect frame :/ \n");
    return 0;
...
void demoKinect(char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes,
    int frame_skip, char *prefix, char *out_filename, int http_stream_port, int dont_show, int ext_output)
{
    //skip = frame_skip;
    image **alphabet = load_alphabet();
...
*/
    ...
    while (1) {
        ++count;
        if (1) {
            if (pthread_create(&fetch_thread, 0, fetch_in_thread_kinect, 0)) error("Thread creation failed");
            if (pthread_create(&detect_thread, 0, detect_in_thread, 0)) error("Thread creation failed");
